...
def apply_security_headers(response):
    ...
    response.headers["Content-Security-Policy"] = (
        ...
        "connect-src 'self' https://curtisconnect.secure-tech.org wss://curtisconnect.secure-tech.org https://cdn.jsdelivr.net https://www.google.com https://firebasestorage.googleapis.com https://storage.googleapis.com; "
        "img-src 'self' data: blob: https://www.gstatic.com https://firebasestorage.googleapis.com https://storage.googleapis.com;"
    )
    return response
...
def upload_file():
    ...
    if file:
        ...
        encrypted_file_info = encrypt_message(json.dumps(file_info), aes_key)

        # Broadcast to all clients in the room
        socketio.emit("message", {
            "user": session.get("username"),
            "msg": encrypted_file_info,
            "roomId": room_id
        }, room=room_id)
